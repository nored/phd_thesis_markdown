...
def pdf():
    """ Generate a PDF file """
    env.file_ext = ".pdf"
    local("pandoc {input_files} -o {output_file}{file_ext} -H {preamble_file} --template {template_file} --bibliography={bib_file} --csl={csl_file} -V fontsize=12pt -V papersize=a4paper -V lang:german -V documentclass:report -N --latex-engine=xelatex".format(**env))
